...
COPY pyproject.toml README.md uv.lock /build/signaldesk-notification-api/
COPY src /build/signaldesk-notification-api/src
RUN python -m pip install uv==0.11.31 \
 && cd /build/signaldesk-notification-api \
 && UV_PROJECT_ENVIRONMENT=/app/.venv uv sync --locked --no-dev --no-editable \
 && rm -rf /build
RUN useradd --uid 10001 --create-home appuser
USER 10001:10001
CMD ["uvicorn", "signaldesk_notification_api.main:create_configured_app", "--factory", "--host", "0.0.0.0", "--port", "8080"]
